def it_was_ok
  ...
  Movie.select('movies.id, movies.title, movies.score').where(score: 2..3)#'movies.score BETWEEN 8 AND 9')
...
def harrison_ford
  ...
  # Movie.select(:id, :title).joins(:castings).joins(:actor).where(actors: { name: 'Harrison Ford', ord: 1 })
  Casting.select('movies.id, movies.title').joins(:actor).joins(:movie).where("actors.name = 'Harrison Ford' AND castings.ord != 1")
end
...
def biggest_cast
  ...
  Casting.select('movies.id, movies.title').joins(:actor).joins(:movie).group('movies.id')
